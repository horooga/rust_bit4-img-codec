...
fn help() {
    println!("Usage: exe [option] [input_file_path(opt)] [output_file_path(opt)] [base64url_key(opt)]

    options:
        - e - encode mode: input - existing [input_file_path], output - created [output_file_path] or stderr
        - p - enable dither processing for image encoding (can be disabled when re-encoding of already dithered images)
        - d - decode mode: input - existing [input_file_path], output - stdout decode text or stderr
        - ee - encode-encrypt mode: input - existing [input_file_path] and [base64url_key], output - created [output_file_path] or stderr
        - dd - decode-decrypt mode: input - existing [input_file_path] and [base64url_key], output - created [output_file_path] or stderr
